...
class Contact extends Component{
    ...
    validation = ()=>{
        if(this.state.name.indexOf(' ') === -1 && this.state.name.length > 0){
            this.setState({nameAccept:1 ,nameColor:'transparent' , nameHrColor:'#737373'})
        }else{
            this.setState({nameColor:'red' , nameHrColor:'red'})
        }
        if(this.state.email.indexOf('@') !== -1 && this.state.email.length >0){
            this.setState({emailAccept:1, emailColor:'transparent', emailHrColor:'#737373'})
        }else{
            this.setState({emailColor:'red', emailHrColor:'red'})
        }
        if(this.state.message.length > 120){
            this.setState({messageAccept:1,messageColor:'transparent', messageHrColor:'#737373'})
        }else{
            ...
            <div className='contact'>
                ...
                <div>
                    ...
                    <form>
                        ...
                        <textarea name='message' value={this.state.message} placeholder='Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec tincidunt ligula a lacinia pulvinar. Pellentesque quis ultricies sem, nec laoreet nisl.Donec tincidunt ligula a lacinia pulvinar.' onChange={this.handleChange}/>
